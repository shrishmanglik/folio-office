...
const definitions = [
  ...
  ['spreadsheet.edit', 'Clear contents or formatting; fill in any direction with relative formulas; replace literal cell text; copy a rectangle within the workbook with paste modes; sort records; or rename, duplicate, move or delete a sheet. Unsafe reference edits are rejected. Sort keyColumn and sheet toIndex are zero-based.', { ...file, ...revision, sheetId:str.optional(),action:z.enum(['copyRange','replace','clear','fill','sort','sheetRename','sheetDuplicate','sheetMove','sheetDelete']),range:z.string().max(32).optional(),target:z.string().regex(/^[A-Z]+[1-9][0-9]*$/).optional(),targetSheetId:str.optional(),transpose:z.boolean().optional(),skipBlanks:z.boolean().optional(),mode:z.enum(['contents','formats','all','values','formulas']).optional(),direction:z.enum(['down','right','up','left','asc','desc']).optional(),find:z.string().min(1).max(10000).optional(),replacement:z.string().max(10000).optional(),matchCase:z.boolean().optional(),wholeCell:z.boolean().optional(),keyColumn:z.number().int().min(0).max(999).optional(),hasHeader:z.boolean().optional(),name:z.string().min(1).max(31).optional(),toIndex:z.number().int().min(0).max(99).optional() }, false, true],
  ['spreadsheet.format', 'Merge validated cell formatting into an A1 range of at most 10000 cells.', { ...file, ...revision, sheetId:str.optional(),range:z.string().regex(/^[A-Z]+[1-9][0-9]*(:[A-Z]+[1-9][0-9]*)?$/),style:z.object({fontFamily:z.string().min(1).max(100).optional(),fontSize:z.number().min(6).max(96).optional(),underline:z.boolean().optional(),strike:z.boolean().optional(),verticalAlign:z.enum(['top','middle','bottom']).optional(),wrap:z.boolean().optional(),indent:z.number().int().min(0).max(15).optional(),rotation:z.number().int().min(-90).max(90).optional(),border:z.enum(['none','all','outside','bottom']).optional(),borderColor:z.string().regex(/^#[0-9a-f]{6}$/i).optional(),bold:z.boolean().optional(),italic:z.boolean().optional(),numberFormat:z.enum(['General','0','0.00','0%','$#,##0.00','0.000','0.00%','#,##0','#,##0.00','0.00E+00','yyyy-mm-dd','@']).optional(),fill:z.string().regex(/^#[0-9a-f]{6}$/i).optional(),color:z.string().regex(/^#[0-9a-f]{6}$/i).optional(),align:z.enum(['left','center','right']).optional()}).strict() }, false, true],
