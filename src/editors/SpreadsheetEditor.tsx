...
function displayValue(value: string, format = 'General'): string {
  if (value === '' || !Number.isFinite(Number(value)) || format === 'General') return value;
...
export default function SpreadsheetEditor({ file, onChange }: EditorProps) {
  ...
  useEffect(() => {
    const dismiss = (event: PointerEvent) => { if (!exportMenu.current?.contains(event.target as Node)) exportMenu.current?.removeAttribute('open'); };
    const escape = (event: KeyboardEvent) => { if (event.key === 'Escape' && exportMenu.current?.open) { exportMenu.current.removeAttribute('open'); exportMenu.current.querySelector('summary')?.focus(); } };
        ...
        book.eachSheet(ws => {
          ...
          ws.eachRow({ includeEmpty: true }, (row, r) => {
            ...
            row.eachCell({ includeEmpty: true }, (cell, c) => {
              ...
              styles[cell.address] = { bold: cell.font?.bold, italic: cell.font?.italic, numberFormat: cell.numFmt, color: cell.font?.color?.argb ? `#${cell.font.color.argb.slice(-6)}` : undefined, fill: fill ? `#${fill.slice(-6)}` : undefined, align: ['left', 'center', 'right'].includes(cell.alignment?.horizontal || '') ? cell.alignment.horizontal as CellStyle['align'] : undefined };
              cells[r - 1][c - 1] = cell.type === ExcelJS.ValueType.Formula ? `=${cell.formula}` : v instanceof Date ? v.toISOString().slice(0, 10) : cell.text;
            });
          });
  ...
  async function exportFile(format: 'csv' | 'xlsx') {
    ...
    try {
      ...
      if (format === 'csv') await downloadFile(file.name, sheet.cells.map(r => r.map(csvEscape).join(',')).join('\r\n'), 'csv', 'text/csv;charset=utf-8');
        ...
        })); for (const [address, style] of Object.entries(s.styles || {})) { const cell = ws.getCell(address); cell.font = { bold: style.bold, italic: style.italic, ...(style.color ? { color: { argb: `FF${style.color.replace('#', '')}` } } : {}) }; if (style.numberFormat) cell.numFmt = style.numberFormat; if (style.fill) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${style.fill.replace('#', '')}` } }; if (style.align) cell.alignment = { horizontal: style.align }; } ws.columns.forEach(c => { c.width = 16; }); }
  ...
  return <div className="spreadsheet-editor">
    ...
    <div className="sheet-format-toolbar" aria-label="Cell formatting">
      ...
      </div><div className="sheet-format-group" role="group" aria-label="Number and alignment"><select aria-label="Number format" value={selectedStyle.numberFormat || 'General'} onChange={e => formatCell({ numberFormat: e.target.value })}>{['General', '0', '0.00', '0%', '$#,##0.00'].map(f => <option key={f}>{f}</option>)}</select>
      <select aria-label="Cell alignment" value={selectedStyle.align || ''} onChange={e => formatCell({ align: e.target.value as CellStyle['align'] })}><option value="">Auto align</option><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select>
      ...
      </div><button onClick={clearFormatting}>Clear format</button>
    </div>
    <div className="sheet-view-toolbar"><span className="sheet-view-label">View</span><input type="search" aria-label="Filter rows" placeholder="Filter rows by value or formulaâ€¦" value={filter} onChange={e => setFilter(e.target.value)}/>{filter && <span>{visibleRows.length} matching rows <button onClick={() => setFilter('')}>Clear</button></span>}<label><input type="checkbox" checked={freezeFirstRow} onChange={e => setFreezeFirstRow(e.target.checked)}/>Freeze first row</label></div>
    <div className="sheet-formula"><input className="sheet-address-input" aria-label="Select cell or range" title="Enter A1:D10, or Shift-click cells" value={rangeDraft} onChange={e=>setRangeDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();selectRange();}}}/><select aria-label="Insert summary formula" value="" onChange={e=>{if(e.target.value)insertSummaryFormula(e.target.value);}}><option value="">Insert function</option>{['SUM','AVERAGE','MIN','MAX','COUNT'].map(f=><option key={f} value={f}>{f}</option>)}</select><span className="sheet-fx">Æ’x</span><input aria-label="Formula bar" value={draft} onChange={e => setDraft(e.target.value)} onBlur={commit} onKeyDown={e => { if (e.key === 'Enter') { commit(); grid.current?.focus(); } if (e.key === 'Escape') { cancelCommit.current = true; queueMicrotask(() => { cancelCommit.current = false; }); setDraft(raw); grid.current?.focus(); } }} placeholder="Enter a value or a formula, e.g. =SUM(A1:A10)"/></div>
    {(message || computed.error) && <div role="status" className="sheet-message">{message || computed.error}<button onClick={() => setMessage('')} aria-label="Dismiss message">Ã—</button></div>}
